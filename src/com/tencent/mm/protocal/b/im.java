package com.tencent.mm.protocal.b;

import java.util.LinkedList;

public final class im
  extends adk
{
  public int cVl;
  public String dse;
  public String hic;
  public int hjV;
  public int hjW;
  public int hjX;
  public int hjv;
  public String hlH;
  public String hsh;
  public int hsi;
  public int hsj;
  
  protected final int a(int paramInt, Object... paramVarArgs)
  {
    if (paramInt == 0)
    {
      paramVarArgs = (a.a.a.c.a)paramVarArgs[0];
      if (hLO != null)
      {
        paramVarArgs.bN(1, hLO.kS());
        hLO.a(paramVarArgs);
      }
      if (hic != null) {
        paramVarArgs.U(2, hic);
      }
      paramVarArgs.bM(3, hjv);
      if (hlH != null) {
        paramVarArgs.U(4, hlH);
      }
      if (dse != null) {
        paramVarArgs.U(5, dse);
      }
      paramVarArgs.bM(6, hjV);
      paramVarArgs.bM(7, hjW);
      paramVarArgs.bM(8, hjX);
      if (hsh != null) {
        paramVarArgs.U(9, hsh);
      }
      paramVarArgs.bM(10, hsi);
      paramVarArgs.bM(11, cVl);
      paramVarArgs.bM(12, hsj);
      return 0;
    }
    if (paramInt == 1) {
      if (hLO == null) {
        break label770;
      }
    }
    label770:
    for (paramInt = a.a.a.a.bJ(1, hLO.kS()) + 0;; paramInt = 0)
    {
      int i = paramInt;
      if (hic != null) {
        i = paramInt + a.a.a.b.b.a.T(2, hic);
      }
      i += a.a.a.a.bI(3, hjv);
      paramInt = i;
      if (hlH != null) {
        paramInt = i + a.a.a.b.b.a.T(4, hlH);
      }
      i = paramInt;
      if (dse != null) {
        i = paramInt + a.a.a.b.b.a.T(5, dse);
      }
      i = i + a.a.a.a.bI(6, hjV) + a.a.a.a.bI(7, hjW) + a.a.a.a.bI(8, hjX);
      paramInt = i;
      if (hsh != null) {
        paramInt = i + a.a.a.b.b.a.T(9, hsh);
      }
      return paramInt + a.a.a.a.bI(10, hsi) + a.a.a.a.bI(11, cVl) + a.a.a.a.bI(12, hsj);
      if (paramInt == 2)
      {
        paramVarArgs = new a.a.a.a.a((byte[])paramVarArgs[0], hfZ);
        for (paramInt = adk.a(paramVarArgs); paramInt > 0; paramInt = adk.a(paramVarArgs)) {
          if (!super.a(paramVarArgs, this, paramInt)) {
            paramVarArgs.aVo();
          }
        }
        break;
      }
      if (paramInt == 3)
      {
        Object localObject1 = (a.a.a.a.a)paramVarArgs[0];
        im localim = (im)paramVarArgs[1];
        paramInt = ((Integer)paramVarArgs[2]).intValue();
        switch (paramInt)
        {
        default: 
          return -1;
        case 1: 
          paramVarArgs = ((a.a.a.a.a)localObject1).pL(paramInt);
          i = paramVarArgs.size();
          paramInt = 0;
          while (paramInt < i)
          {
            Object localObject2 = (byte[])paramVarArgs.get(paramInt);
            localObject1 = new cj();
            localObject2 = new a.a.a.a.a((byte[])localObject2, hfZ);
            for (boolean bool = true; bool; bool = ((cj)localObject1).a((a.a.a.a.a)localObject2, (com.tencent.mm.al.a)localObject1, adk.a((a.a.a.a.a)localObject2))) {}
            hLO = ((cj)localObject1);
            paramInt += 1;
          }
        case 2: 
          hic = jMD.readString();
          return 0;
        case 3: 
          hjv = jMD.aVp();
          return 0;
        case 4: 
          hlH = jMD.readString();
          return 0;
        case 5: 
          dse = jMD.readString();
          return 0;
        case 6: 
          hjV = jMD.aVp();
          return 0;
        case 7: 
          hjW = jMD.aVp();
          return 0;
        case 8: 
          hjX = jMD.aVp();
          return 0;
        case 9: 
          hsh = jMD.readString();
          return 0;
        case 10: 
          hsi = jMD.aVp();
          return 0;
        case 11: 
          cVl = jMD.aVp();
          return 0;
        }
        hsj = jMD.aVp();
        return 0;
      }
      return -1;
    }
  }
}

/* Location:
 * Qualified Name:     com.tencent.mm.protocal.b.im
 * Java Class Version: 6 (50.0)
 * JD-Core Version:    0.7.1
 */
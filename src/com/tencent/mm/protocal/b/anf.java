package com.tencent.mm.protocal.b;

import a.a.a.b;
import java.util.LinkedList;

public final class anf
  extends com.tencent.mm.al.a
{
  public int hSQ;
  public int hSR;
  public adt hSS;
  public int hjW;
  
  protected final int a(int paramInt, Object... paramVarArgs)
  {
    if (paramInt == 0)
    {
      paramVarArgs = (a.a.a.c.a)paramVarArgs[0];
      if (hSS == null) {
        throw new b("Not all required fields were included: PieceData");
      }
      paramVarArgs.bM(1, hjW);
      paramVarArgs.bM(2, hSQ);
      paramVarArgs.bM(3, hSR);
      if (hSS != null)
      {
        paramVarArgs.bN(4, hSS.kS());
        hSS.a(paramVarArgs);
      }
      paramInt = 0;
    }
    int i;
    do
    {
      return paramInt;
      if (paramInt != 1) {
        break;
      }
      i = a.a.a.a.bI(1, hjW) + 0 + a.a.a.a.bI(2, hSQ) + a.a.a.a.bI(3, hSR);
      paramInt = i;
    } while (hSS == null);
    return i + a.a.a.a.bJ(4, hSS.kS());
    if (paramInt == 2)
    {
      paramVarArgs = new a.a.a.a.a((byte[])paramVarArgs[0], hfZ);
      for (paramInt = com.tencent.mm.al.a.a(paramVarArgs); paramInt > 0; paramInt = com.tencent.mm.al.a.a(paramVarArgs)) {
        if (!super.a(paramVarArgs, this, paramInt)) {
          paramVarArgs.aVo();
        }
      }
      if (hSS == null) {
        throw new b("Not all required fields were included: PieceData");
      }
      return 0;
    }
    if (paramInt == 3)
    {
      Object localObject1 = (a.a.a.a.a)paramVarArgs[0];
      anf localanf = (anf)paramVarArgs[1];
      paramInt = ((Integer)paramVarArgs[2]).intValue();
      switch (paramInt)
      {
      default: 
        return -1;
      case 1: 
        hjW = jMD.aVp();
        return 0;
      case 2: 
        hSQ = jMD.aVp();
        return 0;
      case 3: 
        hSR = jMD.aVp();
        return 0;
      }
      paramVarArgs = ((a.a.a.a.a)localObject1).pL(paramInt);
      i = paramVarArgs.size();
      paramInt = 0;
      while (paramInt < i)
      {
        Object localObject2 = (byte[])paramVarArgs.get(paramInt);
        localObject1 = new adt();
        localObject2 = new a.a.a.a.a((byte[])localObject2, hfZ);
        for (boolean bool = true; bool; bool = ((adt)localObject1).a((a.a.a.a.a)localObject2, (com.tencent.mm.al.a)localObject1, com.tencent.mm.al.a.a((a.a.a.a.a)localObject2))) {}
        hSS = ((adt)localObject1);
        paramInt += 1;
      }
      return 0;
    }
    return -1;
  }
}

/* Location:
 * Qualified Name:     com.tencent.mm.protocal.b.anf
 * Java Class Version: 6 (50.0)
 * JD-Core Version:    0.7.1
 */
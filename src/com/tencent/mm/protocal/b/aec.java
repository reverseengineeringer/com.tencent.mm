package com.tencent.mm.protocal.b;

import a.a.a.b;
import java.util.LinkedList;

public final class aec
  extends com.tencent.mm.ax.a
{
  public int Type;
  public String fBO;
  public int fyR;
  public int jYZ;
  public amj juX;
  public String jvc;
  
  protected final int a(int paramInt, Object... paramVarArgs)
  {
    if (paramInt == 0)
    {
      paramVarArgs = (a.a.a.c.a)paramVarArgs[0];
      if (juX == null) {
        throw new b("Not all required fields were included: ToUserName");
      }
      if (juX != null)
      {
        paramVarArgs.cx(1, juX.iO());
        juX.a(paramVarArgs);
      }
      if (fBO != null) {
        paramVarArgs.e(2, fBO);
      }
      paramVarArgs.cw(3, Type);
      paramVarArgs.cw(4, fyR);
      paramVarArgs.cw(5, jYZ);
      if (jvc != null) {
        paramVarArgs.e(6, jvc);
      }
      return 0;
    }
    if (paramInt == 1) {
      if (juX == null) {
        break label532;
      }
    }
    label532:
    for (paramInt = a.a.a.a.cv(1, juX.iO()) + 0;; paramInt = 0)
    {
      int i = paramInt;
      if (fBO != null) {
        i = paramInt + a.a.a.b.b.a.f(2, fBO);
      }
      i = i + a.a.a.a.cu(3, Type) + a.a.a.a.cu(4, fyR) + a.a.a.a.cu(5, jYZ);
      paramInt = i;
      if (jvc != null) {
        paramInt = i + a.a.a.b.b.a.f(6, jvc);
      }
      return paramInt;
      if (paramInt == 2)
      {
        paramVarArgs = new a.a.a.a.a((byte[])paramVarArgs[0], jrk);
        for (paramInt = com.tencent.mm.ax.a.a(paramVarArgs); paramInt > 0; paramInt = com.tencent.mm.ax.a.a(paramVarArgs)) {
          if (!super.a(paramVarArgs, this, paramInt)) {
            paramVarArgs.bve();
          }
        }
        if (juX != null) {
          break;
        }
        throw new b("Not all required fields were included: ToUserName");
      }
      if (paramInt == 3)
      {
        Object localObject1 = (a.a.a.a.a)paramVarArgs[0];
        aec localaec = (aec)paramVarArgs[1];
        paramInt = ((Integer)paramVarArgs[2]).intValue();
        switch (paramInt)
        {
        default: 
          return -1;
        case 1: 
          paramVarArgs = ((a.a.a.a.a)localObject1).vC(paramInt);
          i = paramVarArgs.size();
          paramInt = 0;
          while (paramInt < i)
          {
            Object localObject2 = (byte[])paramVarArgs.get(paramInt);
            localObject1 = new amj();
            localObject2 = new a.a.a.a.a((byte[])localObject2, jrk);
            for (boolean bool = true; bool; bool = ((amj)localObject1).a((a.a.a.a.a)localObject2, (com.tencent.mm.ax.a)localObject1, com.tencent.mm.ax.a.a((a.a.a.a.a)localObject2))) {}
            juX = ((amj)localObject1);
            paramInt += 1;
          }
        case 2: 
          fBO = mMY.readString();
          return 0;
        case 3: 
          Type = mMY.id();
          return 0;
        case 4: 
          fyR = mMY.id();
          return 0;
        case 5: 
          jYZ = mMY.id();
          return 0;
        }
        jvc = mMY.readString();
        return 0;
      }
      return -1;
    }
  }
}

/* Location:
 * Qualified Name:     com.tencent.mm.protocal.b.aec
 * Java Class Version: 6 (50.0)
 * JD-Core Version:    0.7.1
 */